...
static void espnow_task(void *pvParameter)
{
	...
	while (xQueueReceive(s_espnow_queue, &evt, portMAX_DELAY) == pdTRUE) {
		if (evt.id == EXAMPLE_ESPNOW_SEND_CB) {
			...
			//strcpy(mqttBuf.data, recv_data.topic);		//
			if (strcmp(recv_data.topic, "3") == 0)
			{
				sprintf(mqttBuf.data, "{adc3:%s}", recv_data.payload);
			}
			else if (strcmp(recv_data.topic, "2") == 0)
			{
				sprintf(mqttBuf.data, "{adc4:%s}", recv_data.payload);
			}
			//sprintf(mqttBuf.data, "{adc3:%s}", recv_data.payload);
