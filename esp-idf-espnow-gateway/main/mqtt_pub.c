...
void mqtt_pub(void *pvParameters)
{
	...
	esp_mqtt_client_config_t mqtt_cfg = {
        ...
        //.credentials.username = "Fu5SslZFILvfXQ5Y15S2",
        //.credentials.password = "Fu5SslZFILvfXQ5Y15S2",
	};

#else
	esp_mqtt_client_config_t mqtt_cfg = {
		.uri = "mqtt://192.168.0.83",
		.username = "Fu5SslZFILvfXQ5Y15S2",
		.password = "Fu5SslZFILvfXQ5Y15S2",
	...
	while (1) {
		...
		if (EventBits & MQTT_CONNECTED_BIT) {
			if (mqttBuf.topic_type == PUBLISH) {
				...
				ESP_LOGI(TAG, "Funciona: sent publish successful, msg_id=%d", msg_id);
				//ESP_LOGI(TAG, "condicional, sent data: %s to topic: %s", data, topic);
